...
Greedy netting (`money.net_transfers`, ties by lowest id):
`a3→a1 240k`, `a3→a2 145k`, `a5→a2 95k`, `a5→a4 65k` (sums to 0).
...
- Step 9's new a1 draft is **confirmed** before step 10, so Friday's meal is in
  the settlement (matches "second tính tiền includes thursday and friday"). The
  deterministic runner confirms it explicitly.
- "reset balance" (step 11) = `settle_period commit:true`: it records the
  ...
  read zero. No dedicated zero-out tool.
...
- **`ledger.record_payment(...)`** — validates both members belong to the room
  and `amount > 0`; writes one row; returns a summary dict.
- **`ledger.period_balances`** — after summing meals, fold payments in the
  window: `balance[from] += amount`, `balance[to] -= amount`. Payments carry no
...
- **Rendering** — `chat.render_bot_attachments` recognizes `record_payment`;
  ...
  N đ") from the tool dict. Reset/settle rendering unchanged.
...
- **Deterministic runner `tests/test_scenario_week.py`** (CI):
  - Seeds a room (reusing `test_ledger._seed_room`).
  - Iterates STEPS. For each, monkeypatches `app.clock.today_ict`/`now_ict` to
    the step's day, executes the step against the real modules
