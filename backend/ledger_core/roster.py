"""Any roster: read + name/alias/mention resolution, room-scoped.
...
def list_members(
    ...
    does.
    """
    return _members.directory().list(session, room_id, include_inactive=include_inactive,
...
class _NameIndex:
    ...
    **bank account holder** — the last is often the only place a person's real
    ...
    def __init__(self, members: list[Any]):
        ...
        self.exact: dict[str, list[Any]] = {}       # accent-preserving
        self.folded: dict[str, list[Any]] = {}      # tone-stripped
        self.given: dict[str, list[Any]] = {}       # last token = Vietnamese given name
        self.token: dict[str, list[Any]] = {}       # any token, any position
    ...
    @staticmethod
    def _fields(m: Any) -> list[str]:
        return [m.display_name, m.nickname, m.account_holder, *(m.aliases or [])]

    @staticmethod
    def _add(index: dict[str, list[Any]], key: str, m: Any) -> None:
        bucket = index.setdefault(key, [])
        if m.id not in {x.id for x in bucket}:
            bucket.append(m)

    def lookup(self, raw: str) -> list[Any]:
