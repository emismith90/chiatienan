...
``lunch_ledger`` and ``ledger_tools`` are the framework's (``packs/``), registered here
with this host's place resolver, QR builder and bank-memo wording; ``lunch_places`` and ``room_members`` are
...
"""
from app.packs.lunch import ledger_tools_pack, lunch_ledger_pack
