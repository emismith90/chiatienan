...
- **Giving a cloud agent the SSH key does not unblock deploys — don't.** The tempting fix is to base64 `DEPLOY_SSH_KEY` into the cloud environment's *Environment variables* box. It cannot work: the block is at the network layer, not authentication, so port 22 still times out even with the host allowlisted (verified with the domain allowlisted and `:443` fully working). It also puts a root private key somewhere that panel itself warns is "visible to anyone using this environment", to buy nothing. The runner in `deploy.yml` already holds that key as a repo secret and has unrestricted egress — **that** is the deploy path. When SSH is genuinely needed by a human, use a phone hotspot or the DigitalOcean web console.
